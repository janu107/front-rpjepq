...
import {
  Box, Button, Chip, Grid, IconButton, Paper, Stack, Table, TableBody, TableCell,
...
const NominaTiempoExtraPage = () => {
  ...
  const [detalle, setDetalle] = useState({ id: null, rows: [] });
  ...
  const verDetalle = async (row) => {
    try {
      const { data } = await axiosClient.get(`/nomina-tiempo-extra/${row.id}/detalle`);
      setDetalle({ id: row.id, numero: row.numero, rows: data.data || [] });
    } catch (error) {
  ...
  return (
    <Stack spacing={2.5}>
      ...
      {detalle.id && (
        <Box>
          <Typography variant="subtitle1" sx={{ fontWeight: 800, mb: 1 }}>Detalle planilla {detalle.numero}</Typography>
          <TableContainer component={Paper} elevation={0} sx={{ border: "1px solid #dde3ea" }}>
            <Table size="small">
              <TableHead sx={{ bgcolor: "rgba(31,78,95,0.08)" }}>
                <TableRow>
