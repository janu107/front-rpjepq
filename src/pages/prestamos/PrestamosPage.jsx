  ...
  return (
    <Stack spacing={2.5}>
      <Stack direction={{ xs: "column", md: "row" }} spacing={2} justifyContent="space-between" alignItems={{ md: "center" }}>
        <Box><Typography variant="h5">Prestamos</Typography><Typography color="text.secondary">Control de prestamos y pagos</Typography></Box>
        {canCreate(user) && <Button variant="contained" startIcon={<AddIcon />} onClick={openCreate}>Nuevo prestamo</Button>}
      </Stack>
      ...
      <TableContainer component={Paper} elevation={0} sx={{ border: "1px solid #dde3ea" }}>
        <Table><TableHead><TableRow><TableCell>Contrato</TableCell><TableCell>Aportacion</TableCell><TableCell>DPI</TableCell><TableCell>Monto</TableCell><TableCell>Cuota</TableCell><TableCell>Estado</TableCell><TableCell align="right">Acciones</TableCell></TableRow></TableHead>
          <TableBody>{filtered.map((item) => <TableRow key={item.id} hover><TableCell>{item.noContrato}</TableCell><TableCell>{item.aportacionNombre}</TableCell><TableCell>{item.aportacionDpi}</TableCell><TableCell>Q {Number(item.montoAutorizado).toFixed(2)}</TableCell><TableCell>Q {Number(item.cuotaNivelada).toFixed(2)}</TableCell><TableCell><Chip label={item.estado} color={item.estado === "ACTIVO" ? "success" : "default"} size="small" /></TableCell><TableCell align="right">{canEdit(user) && <Tooltip title="Editar"><IconButton color="primary" onClick={() => openEdit(item)}><EditIcon /></IconButton></Tooltip>}<Tooltip title="Detalle"><IconButton color="primary" onClick={() => openDetalle(item)}><ListAltIcon /></IconButton></Tooltip>{canEdit(user) && <Tooltip title="Estado"><IconButton color="primary" onClick={() => changeStatus(item)}><PowerSettingsNewIcon /></IconButton></Tooltip>}{canDelete(user) && <Tooltip title="Eliminar"><IconButton color="primary" onClick={() => deletePrestamo(item)}><DeleteIcon /></IconButton></Tooltip>}</TableCell></TableRow>)}</TableBody>
        </Table>
      </TableContainer>
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="md"><DialogTitle>{editing ? "Editar prestamo" : "Nuevo prestamo"}</DialogTitle><DialogContent><Stack spacing={2} sx={{ mt: 1 }}>
        <FormControl fullWidth><InputLabel>Aportacion</InputLabel><Select label="Aportacion" value={form.idAportacion} onChange={(e) => setForm({ ...form, idAportacion: e.target.value })}>{aportaciones.map((a) => <MenuItem key={a.id} value={a.id}>{a.nombre} {a.apellido} - {a.dpi}</MenuItem>)}</Select></FormControl>
      ...
      </Stack></DialogContent><DialogActions><Button onClick={() => setDialogOpen(false)}>Cancelar</Button><Button variant="contained" onClick={savePrestamo}>Guardar</Button></DialogActions></Dialog>
      <Dialog open={detailOpen} onClose={() => setDetailOpen(false)} fullWidth maxWidth="lg"><DialogTitle>Detalle de prestamo</DialogTitle><DialogContent><Stack spacing={2} sx={{ mt: 1 }}><Typography variant="h6">Total pagado: Q {Number(totalPagado).toFixed(2)} | Saldo actual: Q {Number(saldo).toFixed(2)}</Typography>{canCreate(user) && <Stack direction={{ xs: "column", md: "row" }} spacing={2}><TextField label="Fecha pago" type="date" value={detalleForm.fechaPago} onChange={(e) => setDetalleForm({ ...detalleForm, fechaPago: e.target.value })} InputLabelProps={{ shrink: true }} /><TextField label="Cuota" type="number" value={detalleForm.cuotaNivelada} onChange={(e) => setDetalleForm({ ...detalleForm, cuotaNivelada: e.target.value })} /><TextField label="Amortizacion" type="number" value={detalleForm.amortizacion} onChange={(e) => setDetalleForm({ ...detalleForm, amortizacion: e.target.value })} /><TextField label="Intereses" type="number" value={detalleForm.intereses} onChange={(e) => setDetalleForm({ ...detalleForm, intereses: e.target.value })} /><TextField label="Saldo" type="number" value={detalleForm.saldo} onChange={(e) => setDetalleForm({ ...detalleForm, saldo: e.target.value })} /><Button variant="contained" onClick={saveDetalle}>Agregar</Button></Stack>}<TableContainer component={Paper} elevation={0} sx={{ border: "1px solid #dde3ea" }}><Table><TableHead><TableRow><TableCell>Fecha</TableCell><TableCell>Cuota</TableCell><TableCell>Amortizacion</TableCell><TableCell>Intereses</TableCell><TableCell>Saldo</TableCell><TableCell align="right">Acciones</TableCell></TableRow></TableHead><TableBody>{detalle.map((d) => <TableRow key={d.id}><TableCell>{formatDate(d.fechaPago)}</TableCell><TableCell>Q {Number(d.cuotaNivelada).toFixed(2)}</TableCell><TableCell>Q {Number(d.amortizacion).toFixed(2)}</TableCell><TableCell>Q {Number(d.intereses).toFixed(2)}</TableCell><TableCell>Q {Number(d.saldo).toFixed(2)}</TableCell><TableCell align="right">{canDelete(user) && <IconButton color="primary" onClick={() => deleteDetalle(d)}><DeleteIcon /></IconButton>}</TableCell></TableRow>)}</TableBody></Table></TableContainer></Stack></DialogContent><DialogActions><Button onClick={() => setDetailOpen(false)}>Cerrar</Button></DialogActions></Dialog>
    </Stack>
  );
};
